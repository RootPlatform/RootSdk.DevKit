---
path: app-docs/develop/server/guids.md
audience: app
category: guide
...
```ts
import { RootGuidUtils } from "@rootsdk/server-app";
...
```ts
import { RootGuidUtils, RootGuidType } from "@rootsdk/server-app";
...
```ts
import { WellKnownRootGuids } from "@rootsdk/server-app";
