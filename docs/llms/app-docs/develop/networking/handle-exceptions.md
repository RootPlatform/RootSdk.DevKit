...
Use `RootServerException` to report custom failure information from your server to your client. Use **positive numbers** for your own error codes.

The recommended approach is to define your error codes as an enum in your `.proto` file. The code generator exports them in `gen-shared`, making the same enum available to both server and client code. See [Define a service](define-service.md#error-code-enums) for the proto syntax.

```protobuf
enum SuggestionError {
  SUGGESTION_ERROR_UNSPECIFIED = 0;
  SUGGESTION_ERROR_NOT_FOUND = 1;
  SUGGESTION_ERROR_DUPLICATE_VOTE = 2;
}
```

The code generator strips the enum name prefix. In TypeScript, you reference the values as `SuggestionError.NOT_FOUND` and `SuggestionError.DUPLICATE_VOTE`.
...
export class SuggestionService extends SuggestionServiceBase {
  ...
  async delete(request: SuggestionDeleteRequest, client: Client): Promise<SuggestionDeleteResponse> {
    ...
    if (affectedRows === 0) {
      ...
      throw new RootServerException(SuggestionError.NOT_FOUND, message);
    }
  ...
  const deleteSuggestion = useCallback(async (id: number): Promise<void> => {
    try {
      ...
      if (error instanceof RootServerException) {
        if (error.code === SuggestionError.NOT_FOUND) {
