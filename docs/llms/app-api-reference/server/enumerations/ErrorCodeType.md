---
path: app-api-reference/server/enumerations/ErrorCodeType.md
audience: app
category: reference
summary: Enumeration (Error Handling).
---

## Enumeration Members

### AlreadyExists

> **AlreadyExists**: `3`

### Banned

> **Banned**: `8`

### Blocked

> **Blocked**: `17`

### FailedEmailVerificationCode

> **FailedEmailVerificationCode**: `106`

### FailedEmailVerificationCodeTimeout

> **FailedEmailVerificationCodeTimeout**: `105`

### FailedToAuthenticate

> **FailedToAuthenticate**: `102`

### FailedToCreate

> **FailedToCreate**: `100`

### FailedToParse

> **FailedToParse**: `101`

### FailedToSignUp

> **FailedToSignUp**: `103`

### FailedToUpload

> **FailedToUpload**: `104`

### LimitExceeded

> **LimitExceeded**: `18`

### NoPermissionToAdd

> **NoPermissionToAdd**: `1001`

### NoPermissionToBan

> **NoPermissionToBan**: `1010`

### NoPermissionToCreate

> **NoPermissionToCreate**: `1000`

### NoPermissionToDelete

> **NoPermissionToDelete**: `1004`

### NoPermissionToEdit

> **NoPermissionToEdit**: `1003`

### NoPermissionToKick

> **NoPermissionToKick**: `1009`

### NoPermissionToMove

> **NoPermissionToMove**: `1005`

### NoPermissionToRead

> **NoPermissionToRead**: `1002`

### NoPermissionToSpeak

> **NoPermissionToSpeak**: `1008`

### NoPermissionToType

> **NoPermissionToType**: `1007`

### NoPermissionToUpload

> **NoPermissionToUpload**: `1006`

### NotEnoughMembers

> **NotEnoughMembers**: `5`

### NotFound

> **NotFound**: `2`

### NotMemberOf

> **NotMemberOf**: `6`

### PendingFriendshipRequestedSelf

> **PendingFriendshipRequestedSelf**: `200`

### Policy

> **Policy**: `11`

### PolicyAgeRestricted

> **PolicyAgeRestricted**: `401`

### PolicyViolationWellKnownPassword

> **PolicyViolationWellKnownPassword**: `400`

### RequestedSelf

> **RequestedSelf**: `7`

### RequestValidationFailed

> **RequestValidationFailed**: `300`

### ServerError

> **ServerError**: `1`

### StillProcessing

> **StillProcessing**: `13`

### Timeout

> **Timeout**: `12`

### TooLarge

> **TooLarge**: `10`

### TooManyRequests

> **TooManyRequests**: `9`

### UnAuthenticated

> **UnAuthenticated**: `16`

### Unspecified

> **Unspecified**: `0`

### WrongType

> **WrongType**: `4`