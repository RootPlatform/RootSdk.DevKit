---
path: bot-api-reference/type-aliases/MessageReferenceMaps.md
audience: bot
category: reference
...
> `optional` **channels**: [`MessageReferenceMapChannel`](MessageReferenceMapChannel.md)[]
...
> `optional` **roles**: [`MessageReferenceMapCommunityRole`](MessageReferenceMapCommunityRole.md)[]

Array of roles mentioned in the message. Each entry maps a role ID to its current name. Includes `@All` and `@Here` mentions.

### users?

> `optional` **users**: [`MessageReferenceMapUser`](MessageReferenceMapUser.md)[]
