// ============================================================================
// TEST DRIVER — exercises UuidFetchService from a slash command.
//
...
//   /test-external-http-fetch <method> <userIds> [<base64-json-request>]
//
// The actual UUID is non-deterministic — assertions check shape (`uuid`
// present, length 36) rather than the value.
// ============================================================================
...
const COMMAND = "/test-external-http-fetch";
...
async function onTestCommand(evt: ChannelMessageCreatedEvent): Promise<void> {
  ...
  try {
    ...
    // No request payload decode — FetchUuidRequest is empty, the harness
    // sends nothing in tokens[2]. Other recipes' drivers decode a
    // base64 JSON payload at this point; if you fork this recipe and add
    // request fields, copy that pattern from one of them.
...
async function invoke(method: string, client: Client): Promise<Outcome> {
  try {
    switch (method) {
      case "fetchUuid": {
        const r = await uuidFetchService.fetchUuid({}, client);
        // Flatten to shape-checkable fields. The actual UUID is
        // non-deterministic so we assert on length (UUID v4 is 36 chars
        // including hyphens) and that the field is non-empty.
        return {
          kind: "ok",
          fields: {
            uuid_present: typeof r.uuid === "string" && r.uuid.length > 0,
            uuid_length: r.uuid?.length ?? 0,
            fetched_at_present:
              typeof r.fetchedAt === "string" && r.fetchedAt.length > 0,
          },
        };
      }
      default:
