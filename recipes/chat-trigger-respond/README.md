...
# From this directory
npm install
npm run build       # builds server only — no networking, no client

# Then in one terminal (set DEV_TOKEN in server/.env first):
cd server && npm run server      # devhost
```

This recipe has no client to run. To exercise it, type `/ping hello` in any channel of the test community; the app posts `pong: hello` in reply.
