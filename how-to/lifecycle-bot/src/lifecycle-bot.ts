...
// Permissions: channel.createMessage (only for the /lifecycle command)
// Events: none
// Works in: Apps (@rootsdk/server-app) and Bots (@rootsdk/server-bot)
//           Apps use RootAppLifecycle (adds addService) and RootAppStartState
//           (adds channelId). A separate lifecycle-app how-to covers those.
// ============================================================================
//
...
// The /lifecycle command reports this to the channel.
...
async function onLifecycleCommand(evt: ChannelMessageCreatedEvent): Promise<void> {
  ...
  if (!content.startsWith("/lifecycle")) return;
