...
- `channel.createMessage` — only for the `/server-emojis` command trigger
...
- **No create method in the SDK** — emojis are created through the Root platform UI or test harness (community-builder). The `/server-emojis` command requires pre-existing emojis.
