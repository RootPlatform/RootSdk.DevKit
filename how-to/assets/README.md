...
- `channel.createMessage` — only for the `/assets` command trigger
...
**`AssetClient` is only available in `@rootsdk/server-app`** — it is not part of `@rootsdk/server-bot`. This is the key difference that makes assets an app-only how-to.
