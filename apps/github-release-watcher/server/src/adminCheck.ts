...
// (The admin-only RepoListChanged event carries per-row state and has a
// different audience — see releaseWatcherService.ts for the split rationale.)
// ============================================================================
...
export async function isAdmin(userId: UserGuid): Promise<boolean> {
  ...
  // otherwise propagate up through GetFeed / GetSettings and surface as a
  // full QueryError view. Returning false instead degrades gracefully:
...
// startup state). Used by adminAudience.syncAdminAudience() to compose
// the broadcast audience (admins.userIds ∪ owner). NOT for authorization
// checks — use isAdmin for that; it includes the owner fallback.
export function getAdminsGroup(): ReadOnlyMemberGroup | undefined {
  return readAdmins();
}

// Returns the cached community owner's userId, or undefined if
// initializeAdminCheck hasn't run yet. Used by adminAudience to include
// the owner in the broadcast audience even when they aren't explicitly
// listed in globalSettings.general.admins — without this, an owner who
// is admin-by-implicit-fallback wouldn't receive admin-only broadcasts
// and their Settings view would stay stale on their own mutations.
