...
//
// Owner is cached ONCE at startup. Root Platform does not currently
// support runtime ownership transfer; if it does in the future, this
// cache will need to refresh on whatever event signals the change
// (subscribe alongside the existing globalSettings.update wiring).
// The admins reference IS re-captured on every globalSettings "update"
// event.
//
...
export async function isAdmin(userId: UserGuid): Promise<boolean> {
  ...
  // otherwise propagate up through getCanvas/requireAdmin and surface as
...
// All broadcasts in this sample target the public "all" audience, so the
// admins MemberGroup isn't needed as a gated audience. Apps with admin-
// only payloads (see leveling-leaderboard's SettingsUpdated) accessor-
// expose the captured `adminsGroup` ref and pass it as the broadcast
// audience — see that sample's adminCheck.getAdminsGroup() for the
// pattern.
