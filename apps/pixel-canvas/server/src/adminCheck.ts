...
// This sample's picker config is public, so a single broadcast (AdminsChanged)
// covers all clients — see DESIGN.md "Broadcasts" for the conditional split
// rationale apps with admin-only payloads use.
// ============================================================================
...
export async function isAdmin(userId: UserGuid): Promise<boolean> {
  ...
  // until the SDK recovers and the next isAdmin call succeeds. Logged so a
  // real failure is visible to operators.
  try {
...
// globalSettings hasn't surfaced one yet. Used as a broadcast audience for
// admin-only events — NOT for authorization checks (use isAdmin for that;
// it includes the owner fallback).
//
