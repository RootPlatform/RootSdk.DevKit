...
// a spurious 0 (which the parent's auto-save would then clamp up to `min`,
// stomping the user's edit). Commits a parsed (and floored) integer to the
// parent only when the input text parses to a finite in-range number. On
// blur, clamps to [min, max] and restores the last committed value if the
// field is empty or invalid.
//
// External-value sync: when the parent's `value` changes (admin pasted a
// new value, an external SettingsChanged broadcast updated state, etc.)
// we ONLY sync the display when the user isn't actively focused on this
// input. Without this guard, typing "030" while a broadcast arrives mid-
// keystroke would wipe the intermediate state and replace it with the
// broadcast's value. On blur, the standard clamp/restore path fires and
// the display catches up to whatever the parent now holds.
//
// Same shape as the equivalent component in leveling-leaderboard — kept
// in sync as a deliberate convergence point so the cooldown UX feels the
// same across DevKit samples that surface a numeric range setting.
// ============================================================================
...
export const NumberInput: React.FC<Props> = ({ label, value, onChange, min, max, hint }) => {
  ...
  // typing into this input. See header comment for why.
  useEffect(() => {
