...
interface Props {
  ...
  // Optional extra context passed to the logger. Helpful when the same boundary
  // wraps multiple tabs to disambiguate which one threw.
  label?: string;
}
...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo): void {
    ...
    // Fire-and-forget telemetry. The server's reportClientError handler
    // funnels this into structured log output so the crash is visible in
    // the production log aggregator. Errors from the report itself are
    // swallowed — there's no useful UI for "couldn't tell the server".
    void leaderboardServiceClient
      .reportClientError({
        label,
