...
description: Create, get, list, edit, move, and delete channel groups that organize channels
domain: Channel groups
key_methods: [create, update, delete, list, reorder]
---

# API Sample: Channel Groups
...
- **eventHandlers on edit()** — the only channel group method that accepts eventHandlers. Editing access rules can cause groups to gain or lose visibility. In this context, "created"/"deleted" mean visibility changes, not actual creation/deletion. See the `channels/` api sample for a full explanation of the eventHandlers pattern.
- **accessRuleCreates / accessRuleUpdate** — create and edit support inline access rule changes. See the `access-rules/` api sample for details.
