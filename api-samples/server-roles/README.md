...
- `channel.createMessage` — only for the `/server-roles` command trigger, not for role operations
...
- **Roles define permissions; member-roles assign them** — see the `member-roles/` api sample for assigning roles to members.
